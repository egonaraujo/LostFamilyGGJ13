    ...
    class MenuBar : GObject
    {
        ...
        public MenuBar(Texture2D[] button_texture, Texture2D background, int x, int y)
        {
            ...
            y =  y / 2 - NUMBER_OF_BUTTONS / 2 * BUTTON_HEIGHT -
                (NUMBER_OF_BUTTONS % 2) * BUTTON_HEIGHT / 2+64;
        ...
        private void update_buttons()
        {
            for (int i = 0; i < NUMBER_OF_BUTTONS; i++)
            {
                ...
                if (hit_image(
                    button_rectangle[i].X, button_rectangle[i].Y, button_texture[i], mx, my))
                {
                    button_timer[i] = 0.0;
                    if (mpressed)
                    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            {
                rect2.X = 0; rect2.Y = 0;
                rect2.Width = button_texture[i].Width / 2; rect2.Height = button_texture[i].Height;
                //spriteBatch.Draw(button_texture[i], button_rectangle[i], button_color[i]);
                spriteBatch.Draw(button_texture[i], button_rectangle[i], rect2, button_color[i]);
            }
            spriteBatch.End();
        }
